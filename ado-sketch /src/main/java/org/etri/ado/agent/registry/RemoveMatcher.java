...
public class RemoveMatcher extends AbstractActor {
	...
	private boolean isResponseToRemoveItem(GetResponse<?> response) {
		return response.key().equals(AgentRegistry.idToAgentKey) && (response.getRequest().orElse(null) instanceof Remove.Context);
	}	
	
	private void receiveRemove(Remove g) {
		Optional<Object> ctx = Optional.of(new Remove.Context(g.id, null));
		m_replicator.tell(new Replicator.Get<>(AgentRegistry.idToAgentKey, AgentRegistry.readMajority, ctx), self());		
	}
	...
	private void removeAgent(String id, AgentInfo info) {
		Optional<Object> ctx = Optional.of(new RemoveContext(AgentRegistry.idToAgentKey.id(), info));		
		Update<LWWMap<String, AgentInfo>> remove1 = new Update<>(AgentRegistry.idToAgentKey, LWWMap.create(), 
				AgentRegistry.writeMajority, ctx, map -> removeIdToAgents(map, info));
		m_replicator.tell(remove1, self());		
		
		ctx = Optional.of(new RemoveContext(AgentRegistry.roleToAgentKey.id(), info));		
		Update<ORMultiMap<String, AgentInfo>> remove2 = new Update<>(AgentRegistry.roleToAgentKey, ORMultiMap.create(),
				AgentRegistry.writeMajority, ctx, map -> removeRoleToAgents(map, info));
		m_replicator.tell(remove2, self());
		
		ctx = Optional.of(new RemoveContext(AgentRegistry.capaToAgentKey.id(), info));		
		Update<ORMultiMap<String, AgentInfo>> remove3 = new Update<>(AgentRegistry.capaToAgentKey, ORMultiMap.create(),
				AgentRegistry.writeMajority, ctx, map -> removeCapaToAgents(map, info));
