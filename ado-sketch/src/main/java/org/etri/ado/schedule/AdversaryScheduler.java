...
public class AdversaryScheduler extends AbstractActor {
	...
	public static Props prop(AgentSystem system, long interval) {
		return Props.create(AdversaryScheduler.class, system, interval);
	}
	...
	public AdversaryScheduler(AgentSystem system, long interval) {
		m_system = system;
		m_interval = interval;
	}
	...
	public void preStart() {
		try {
			String simpleMlp = new ClassPathResource("prey.h5").getFile().getPath();
	...
	private void receiveTick(Tick tick) {
		...
		Optional<Pair<Float, Float>> agent_vel = getObservation("agent3-velocity");
		if ( !agent_vel.isPresent() ) return;
				
		obs[0][0] = agent_vel.get().getValue0();
		obs[0][1] = agent_vel.get().getValue1();	
		
		Optional<Pair<Float, Float>> agent_loc = getObservation("agent3-loc");
		...
		Optional<Pair<Float, Float>> agent1_loc = getObservation("agent1-loc");
		if ( !agent1_loc.isPresent() ) return;
		
		obs[0][6] = agent1_loc.get().getValue0() - obs[0][2];
		obs[0][7] = agent1_loc.get().getValue1() - obs[0][3];		
		
		Optional<Pair<Float, Float>> agent2_loc = getObservation("agent2-loc");
		if ( !agent2_loc.isPresent() ) return;
		
		obs[0][8] = agent2_loc.get().getValue0() - obs[0][2];
		obs[0][9] = agent2_loc.get().getValue1() - obs[0][3];	
