...
public class GoodAgent2Scheduler extends AbstractActor {
	...
	public void preStart() {
		try {
			String simpleMlp = new ClassPathResource("good2.h5").getFile().getPath();
	...
	private void receiveTick(Tick tick) {
		...
		Optional<Pair<Float, Float>> agent_vel = getObservation("agent2-velocity");
		if ( !agent_vel.isPresent() ) return;
		obs[0][0] = agent_vel.get().getValue0();
		obs[0][1] = agent_vel.get().getValue1();
		
		Optional<Pair<Float, Float>> agent_loc = getObservation("agent2-loc");
