...
public class RemoveMatcher <T extends Tuple> extends AbstractActor {
	...
	private void receiveRemove(Remove rm) {
		...
		m_replicator.tell(new Replicator.Get<>(m_dataKey, TupleSpace.readMajority, ctx), self());
	}
	...
	private void removeTuple(String key) {
		Update<LWWMap<String,T>> update = new Update<>(m_dataKey, LWWMap.create(), TupleSpace.writeMajority,
