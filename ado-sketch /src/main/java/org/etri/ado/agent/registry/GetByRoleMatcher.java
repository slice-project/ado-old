...
public class GetByRoleMatcher extends AbstractActor {
	...
	private void receiveGetByRole(GetByRole g) {
		...
		m_replicator.tell(new Replicator.Get<>(AgentRegistry.roleToAgentKey, AgentRegistry.readMajority, ctx), self());
	}

	private boolean isResponseToGetByRole(GetResponse<?> response) {
		return response.key().equals(AgentRegistry.roleToAgentKey)
	...
	private void receiveGetByRoleFailure(GetFailure<ORMultiMap<String, AgentInfo>> f) {
		...
		m_replicator.tell(new Replicator.Get<>(AgentRegistry.roleToAgentKey, Replicator.readLocal(), ctx), self());
	}	
}
