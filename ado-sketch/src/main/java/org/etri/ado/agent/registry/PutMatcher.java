...
public class PutMatcher extends AbstractActor {
	...
	private void receivePut(Put cmd) {
		...
		Update<LWWMap<String, AgentInfo>> update1 = new Update<>(AgentRegistry.idToAgentKey, LWWMap.create(), 
				AgentRegistry.writeMajority, ctx, map -> updateIdToAgents(map, cmd.agent));
		m_replicator.tell(update1, self());
		
		ctx = Optional.of(new UpdateContext(UpdateContext.BY_ROLE, cmd.agent));
		Update<ORMultiMap<String, AgentInfo>> update2 = new Update<>(AgentRegistry.roleToAgentKey, ORMultiMap.create(),
				AgentRegistry.writeMajority, ctx, map -> updateRoleToAgents(map, cmd.agent));
		m_replicator.tell(update2, self());
		
		ctx = Optional.of(new UpdateContext(UpdateContext.BY_CAP, cmd.agent));
		Update<ORMultiMap<String, AgentInfo>> update3 = new Update<>(AgentRegistry.capaToAgentKey, ORMultiMap.create(),
				AgentRegistry.writeMajority, ctx, map -> updateCapaToAgents(map, cmd.agent));
