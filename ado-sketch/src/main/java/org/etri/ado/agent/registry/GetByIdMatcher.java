...
public class GetByIdMatcher extends AbstractActor {
	...
	private boolean isResponseToGetById(GetResponse<?> response) {
		return response.key().equals(AgentRegistry.idToAgentKey) && (response.getRequest().orElse(null) instanceof GetById.Context);
	}
	...
	private void receiveGetByIdFailure(GetFailure<LWWMap<String, AgentInfo>> f) {
		...
		m_replicator.tell(new Replicator.Get<>(AgentRegistry.idToAgentKey, Replicator.readLocal(), ctx), self());
	}
}
