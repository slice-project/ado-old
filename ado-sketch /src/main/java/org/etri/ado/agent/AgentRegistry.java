...
public class AgentRegistry extends AbstractActor {
	...
	public static class Remove implements Serializable {
		private static final long serialVersionUID = -7525721099069307486L;
		public final String id;
		...
		private static class Context {
			...
			private Context(String id, AgentInfo info) {
				this.id = id;
				this.info = info;
			}
		}		
	...
	public static class UpdateContext {
		...
		private static final String BY_CAP = "by_capabilities";
	...
	public Receive createReceive() {
		return matchGetAll().orElse(matchGetById()).orElse(matchGetByCapabilities()).orElse(matchPut())
				.orElse(matchRemove()).orElse(matchOther());
	}
	...
	private void receivePut(Put cmd) {
		...
		Update<ORMultiMap<String, AgentInfo>> update2 = new Update<>(m_capaToAgentKey, ORMultiMap.create(),
				writeMajority, ctx, map -> updateCapaToAgents(map, cmd.agent));
	...
	private boolean isResponseToGetByCapabilities(GetResponse<?> response) {
		return response.key().equals(m_capaToAgentKey)
	...
	private Receive matchRemove() {
		return receiveBuilder().match(Remove.class, this::receiveRemove)
				.match(GetSuccess.class, this::isResponseToRemoveItem,
						g -> receiveRemoveSuccess((GetSuccess<LWWMap<String, AgentInfo>>) g))
				.match(GetFailure.class, this::isResponseToRemoveItem,
						f -> receiveRemoveFailure((GetFailure<LWWMap<String, AgentInfo>>) f))
				.match(NotFound.class, this::isResponseToRemoveItem, n -> {
					/* nothing to remove */})
				.build();
	}

	private boolean isResponseToRemoveItem(GetResponse<?> response) {
		return response.key().equals(m_idToAgentKey) && (response.getRequest().orElse(null) instanceof Remove.Context);
	}	
	...
	private void removeAgent(String id, AgentInfo info) {
		Optional<Object> ctx = Optional.of(new UpdateContext(UpdateContext.REMOVE, info));
		Update<LWWMap<String, AgentInfo>> update = new Update<>(m_idToAgentKey, LWWMap.create(), writeMajority, ctx,
				map -> map.remove(m_node, id));
	...
	private Receive matchOther() {
		return receiveBuilder().match(UpdateSuccess.class, u -> {
			...
			if ( ctx.type.equals(UpdateContext.REMOVE) ) {
				logger.info("removed  - " + ctx.info);
			}
			else {
